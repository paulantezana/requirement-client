...
export default {
    ...
    reducers: {
        ...
        addList(state, { payload }) {
            ...
            if (
                state.list.find(require => require.product_id === payload.product_id) != undefined
            ) {
                Modal.warning({
        ...
        updateItemByID(state, { payload }) {
            ...
            const newList = state.list.map(
                require => (require.id === payload.id ? { ...require, ...payload } : require)
            );
            return { ...state, list: newList };
        },
        updateItemByProductID(state, { payload }) {
            ...
            const newList = state.list.map(
                require =>
                    require.product_id === payload.product_id ? { ...require, ...payload } : require
            );
            return { ...state, list: newList };
        },
